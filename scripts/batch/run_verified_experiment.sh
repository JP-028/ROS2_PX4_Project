...
if ! ros2 topic info /uav/local_pose 2>/dev/null | grep -Eq "Publisher count: [1-9][0-9]*"; then
  echo "[ERROR] No active publisher for /uav/local_pose."
  echo "Start first in another terminal:"
  echo "  ./scripts/control/start_local_pose_from_px4.sh"
  exit 1
fi

echo "[OK] /uav/local_pose publisher is available."
...
read -rp "Number of runs [default: 5]: " RUN_COUNT
