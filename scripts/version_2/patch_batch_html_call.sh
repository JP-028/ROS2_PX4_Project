...
TARGET="scripts/version_2/run_batch_experiment.sh"
...
p = Path("scripts/version_2/run_batch_experiment.sh")
s = p.read_text()

old = '''python3 scripts/version_2/batch_collect_summary.py "$EXPERIMENT_DIR" \\
  > "$SUMMARY_DIR/batch_collect_summary.log" 2>&1

cat "$SUMMARY_DIR/batch_collect_summary.log"
'''

new = '''python3 scripts/version_2/batch_collect_summary.py "$EXPERIMENT_DIR" \\
  > "$SUMMARY_DIR/batch_collect_summary.log" 2>&1
